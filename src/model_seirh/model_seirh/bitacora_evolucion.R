...
archivo_de_salida_filepath <- "res14/sim_SEIRHUF.csv" # parametros$filepaths$bitacora
...
leer_ultima_linea <- function(){
  ...
  linn <-readLines(conn)
  ultima_linea <- linn[length(linn)]
  #print(ultima_linea)
  close(conn)
  
  #ultima_linea <- system(glue("tail -n 1 {archivo_de_salida_filepath}"), intern = TRUE, show.output.on.console = FALSE)
  #ultima_linea <- system2(glue("tail -n 1 {archivo_de_salida_filepath}"), intern = TRUE)
  #ultima_linea <- system(glue("Get-Content {archivo_de_salida_filepath} -Tail 1"), intern = TRUE)
  return(ultima_linea)
...
importar_datos_desde_ultima_linea <- function(){
  ...
  O <- as.num(datos_raw[28])
  
  dailyR <- as.num(datos_raw[29])
  dailyF <- as.num(datos_raw[30])
  
  R_sin_subRegistro      <- as.num(datos_raw[31])
  dailyR_sin_subRegistro <- as.num(datos_raw[32])
  ...
  establecer_SEIRHUF(S, E, I, R, H, U, F, O)
...
cargar_fitmodel_seirh <- function(.fitmodel, .pars, .n, .probfile) {
  ...
  utils::write.table(fit_summary, file = .probfile, sep=",", col.names = FALSE, 
                     append = TRUE)
